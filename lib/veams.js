...
function getConfigFile() {
	return getPackageJSON().veams.configFile;
}

function getPackageJSON() {
	return JSON.parse(fsx.readFileSync(process.cwd() + '/package.json', 'utf-8'));
}
...
function bowerInstall(component, opts, cb) {
	...
	var isVeamsDep = config.dependencies.veams;
	var veamsBowerVersions = isVeamsDep ? '#~3' : '#~2';

	console.log('veamsBowerVersions: ', veamsBowerVersions, isVeamsDep, 'bower install ' + component + ' ' + veamsBowerVersions + ' ' + opts);
	...
	exec('bower install "' + component + veamsBowerVersions + '" ' + opts, callback);
}
...
function addBlueprintFiles(obj) {
	var folderStructure = Veams.DATA.projectConfig().folderStructure || 'standard';
	var keepStructure = Veams.DATA.projectConfig().keepFolderStructureByInstallation || false;
	...
	if (folderStructure === 'self-contained') {
