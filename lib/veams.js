...
function getImportFile(type) {
	return `${process.cwd()}/${Veams.DATA.projectConfig().importFiles[type]}`;
}
...
async function updateImportFiles(scaffoldPath, name) {
	...
	const scriptPath = `${helpers.resolveImportPath(scriptFile, importScriptPath)}/${path.basename(importScriptPath)}`;
	const updatedScriptFile = await scriptParser(scriptContent, { clName: helpers.capitalizeFirstLetter(helpers.toCamelCase(name)), path: scriptPath});
