...
function addBlueprintFiles(obj) {
	...
	try {
		copyFile(obj.path + '/partials', veams.DATA.projectConfig().paths[bpType] + '/' + obj.name, false);
		copyFile(obj.path + '/data', veams.DATA.projectConfig().paths[bpType] + '/' + obj.name, false);
		copyFile(obj.path + '/scss', veams.DATA.projectConfig().paths.scss + '/' + bpType + 's/', false);
		copyFile(obj.path + '/js', veams.DATA.projectConfig().paths.js + '/' + '/modules/' + obj.name, false);
 ...
 *
 * @param {String} root - root path of files
 * @param {String} dest - destination path of files
 * @param {Boolean} msg - control display of message
 */
function copyFile(root, dest, msg) {
	try {
		fsx.copySync(root, dest);

		if (msg) {
			Helpers.message('green', Helpers.msg.success('Files in ' + root + ' '));
		}
